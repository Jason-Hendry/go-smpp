...
func TestSubmitSMFromRawData(t *testing.T) {
	...
	if pdu.command_length != 65 {
		t.Error("Expected pdu.command_length 65", pdu.command_length)
	}
	...
	if string(pdu.short_message) != short_message {
		t.Error(fmt.Sprintf("Expected short_message to match %s got ",short_message), string(pdu.short_message))
	}
...
func TestBindResp(t *testing.T) {
	data := []byte{0x00,0x00,0x00,0x11,
		...
		0x00}
	pdu := RawPdu(data)
	if(pdu.command_id != PDU_COMMAND_RESP+PDU_COMMAND_BIND_TX) {
		t.Error("Expected command id TX RESP got ",pdu.command_id)
	}
	if(string(pdu.system_type) != "") {
		t.Error("Expected system type empty string got",string(pdu.system_type))
	}
}
