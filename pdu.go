...
func unpackBindBody(raw []byte, output *Pdu) {
	...
	output.interfaceVersion = int(raw[offset])
	offset++
	output.addrTon = int(raw[offset])
	offset++
	output.addrNpi = int(raw[offset])
	offset++

	output.addressRange, _ = UnpackCOctetString(raw, offset, 41)
}
...
func unpackSubmitBodyResp(raw []byte, output *Pdu) {
	...
	output.MessageId, offset = UnpackCOctetString(raw, offset, 65)
}
...
func packBindBody(pdu Pdu) []byte {
	...
	body = append(body, byte(pdu.interfaceVersion))
	body = append(body, byte(pdu.addrTon))
	body = append(body, byte(pdu.addrNpi))

	appendCOctetString(&body, pdu.addressRange)
...
func packSubmitRespBody(pdu Pdu) []byte {
	...
	appendCOctetString(&body, pdu.MessageId)
...
func Bind(sequenceNumber uint32, command uint32, systemId string, password string, systemType string, interfaceVersion int, addrTon int, addrNpi int, addressRange string) Pdu {
	var bind Pdu
	bind.commandId = command
	bind.CommandStatus = 0
	bind.sequenceNumber = sequenceNumber

	bind.systemId = []byte(systemId)
	bind.password = []byte(password)
	bind.systemType = []byte(systemType)
	bind.interfaceVersion = interfaceVersion
	bind.addrTon = addrTon
	bind.addrNpi = addrNpi
	bind.addressRange = []byte(addressRange)
	return bind
}

func SubmitSM(sequenceNumber uint32, systemType string, sourceAddrTon int, sourceAddrNpi int, sourceAddr string,
	destAddrTon int, destAddrNpi int, destAddr string, dataCoding int, smDefaultMsgId int, shortMessage string, optionalParameter []Parameter) Pdu {
	var submit Pdu
	submit.commandId = PDU_COMMAND_SUBMIT_SM
	submit.CommandStatus = 0
	submit.sequenceNumber = sequenceNumber

	submit.systemType = []byte(systemType)
	submit.sourceAddrTon = sourceAddrTon
	submit.sourceAddrNpi = sourceAddrNpi
	submit.sourceAddr = []byte(sourceAddr)

	submit.destAddrTon = destAddrTon
	submit.destAddrNpi = destAddrNpi
	submit.destAddr = []byte(destAddr)
	...
	submit.smDefaultMsgId = smDefaultMsgId

	submit.smLength = 0
	submit.shortMessage = []byte(shortMessage)

	for _, v := range optionalParameter {
		submit.addOptionalParameter(v.tag, v.value)
	}

	return submit
}

func BindResp(pdu Pdu, commandStatus uint32, systemId string) Pdu {
	var bindResp Pdu
	bindResp.commandId = pdu.commandId + PDU_COMMAND_RESP
	bindResp.CommandStatus = commandStatus
	bindResp.sequenceNumber = pdu.sequenceNumber
	bindResp.systemId = []byte(systemId)
	return bindResp
}

func SubmitResp(pdu Pdu, commandStatus uint32, messageId string) Pdu {
	var submitResp Pdu
	submitResp.commandId = pdu.commandId + PDU_COMMAND_RESP
	submitResp.CommandStatus = commandStatus
	submitResp.sequenceNumber = pdu.sequenceNumber
	submitResp.MessageId = []byte(messageId)
	return submitResp
}

func EnquireLink(sequenceNumber uint32) Pdu {
	var bindResp Pdu
	bindResp.commandId = PDU_COMMAND_ENQUIRE
	bindResp.CommandStatus = 0
	bindResp.sequenceNumber = sequenceNumber
	return bindResp
}
...
func (p *Pdu) GetDestination() string {
	return string(p.destAddr)
}

func (p *Pdu) GetSource() string {
	return string(p.sourceAddr)
}

func (p *Pdu) GetMessage() string {
	return string(p.shortMessage)
}

func (p *Pdu) GetOptionalParameters() []Parameter {
	return p.optionalParameters
}
