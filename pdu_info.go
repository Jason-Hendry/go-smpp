...
func (p *Pdu) PrintDetailed() string {
	...
	display = fmt.Sprintf("%sCommand: %s\n", display, pduCommandLabel[int(p.commandId)])
	display = fmt.Sprintf("%sCommand Status: %s\n", display, pduCommandStatusLabel[int(p.CommandStatus)])
	display = fmt.Sprintf("%sSequence: %d\n", display, p.sequenceNumber)
...
func (p *Pdu) printBindBody() string {
	...
	display = fmt.Sprintf("%sSystem Type: %s\n", display, string(p.systemType))

	display = fmt.Sprintf("%sInterface Version: %d\n", display, p.interfaceVersion)
	display = fmt.Sprintf("%sAddr TON: %d (%s)\n", display, p.addrTon, pduTonLabel[int(p.addrTon)])
	display = fmt.Sprintf("%sAddr TON: %d (%s)\n", display, p.addrNpi, pduNpiLabel[int(p.addrNpi)])

	display = fmt.Sprintf("%sAddress Range: %s\n", display, string(p.addressRange))
	return display
}
...
func (p *Pdu) PrintOneLine() string {
	...
	display = fmt.Sprintf("%s (%d) \"%s\" #%d", pduCommandLabel[int(p.commandId)], p.commandLength, pduCommandStatusLabel[int(p.CommandStatus)], p.sequenceNumber)
...
func (p *Pdu) printOneLineBindBody() string {
	var display string
	display = fmt.Sprintf("Id: %s", string(p.systemId))
	display = fmt.Sprintf("%s System Type: %s", display, string(p.systemType))

	display = fmt.Sprintf("%s v:%d", display, p.interfaceVersion)
	display = fmt.Sprintf("%s (TON/NPI) %d/%d", display, p.addrTon, p.addrNpi)

	display = fmt.Sprintf("%s [%s]", display, string(p.addressRange))
	return display
}

func (p *Pdu) printOneLineSubmitBody() string {
	var display string
	display = fmt.Sprintf("System Type: %s", string(p.systemType))
	display = fmt.Sprintf("%s src:%s (%d/%d)", display, string(p.sourceAddr), p.sourceAddrTon, p.sourceAddrNpi)
	display = fmt.Sprintf("%s dst:%s (%d/%d)", display, string(p.destAddr), p.destAddrTon, p.destAddrNpi)

	display = fmt.Sprintf("%s esm/proto/pri:%d/%d/%d", display, p.esmClass, p.protocolId, p.priorityFlag)

	display = fmt.Sprintf("%s sch/valid:%s/%s", display, string(p.scheduleDeliveryTime), string(p.validityPeriod))

	display = fmt.Sprintf("%s reg/rep/dcs:%d/%d/%d", display, p.registeredDelivery, p.replaceIfPresentFlag, p.dataCoding)
	display = fmt.Sprintf("%s %s", display, p.shortMessage)
